function h2mat = Mat2H2_RPY_Proxy(htree, Yp, type, par, alpha, JIT_flag, eta)
%
...
for i = nlevel : -1 : minlvl  
    ...
    for j = 1 : length(level{i})
        ...
        tmpcoord = coord(I{node},:);        
        tmpcoord(:, 1:3) = bsxfun(@minus, tmpcoord(:,1:3), boxcenter);

        %   Row ID for the block rows.
        A_sam = rpy_mex_general({tmpcoord, [Yp{i}, zeros(size(Yp{i},1),1)]}, eta);
        if kdim == 1
...
if JIT_flag == false
    ...
    for i = 1 : length(leafnode)
        ...
        D{node, node} = rpy_mex_general(coord(idx,:), eta);
    end
    ...
    for i = 1 : size(near, 1)
        ...
        D{c1, c2} = rpy_mex_general({coord(idx1, :), coord(idx2, :)}, eta);
    end   
...
h2mat.type = type;
h2mat.par = par;
h2mat.kernel = @(coord)rpy_mex_general(coord, eta);
h2mat.kdim = kdim;
h2mat.Yp = Yp;
